...
class backend:
    ...
    # 		# self.mydb = mysql.connector.connect(user='root', password='40@Vaibhav',host='127.0.0.1', database='dbms')
    ...
    # =======
    # >>>>>>> f89cecae
    # 8f7d11b88564107b1ef1afca091a26f8
    mycursor = ''
    mydb = ''

    def __init__(self):
        self.mydb = mysql.connector.connect(
            host="localhost", user="root", passwd="40@Vaibhav", database='dbms')
        # self.mydb = mysql.connector.connect(user='rhythm', password='password',host='127.0.0.1', database='proj')
        self.mycursor = self.mydb.cursor(buffered=True)
    ...
    def getmovies(self, search_text, filter):
        ...
        if filter == "Name":
            self.mycursor.execute(
                "select * from Movies where Movie_name LIKE '%"+search_text+"%'")
        elif filter == "Age":
            self.mycursor.execute(
                "select * from Movies where Age LIKE '%"+search_text+"%'")
        else:
            self.mycursor.execute(
                "select * from Movies where Genres LIKE '%"+search_text+"%'")
    ...
    def getHallInfo(self, hall_id):
        self.mycursor.execute(
            "select * from hall where Hall_id = " + str(hall_id))
        myresult = self.mycursor.fetchall()
        for hall in myresult:
            hall_info = {
                "hall_name": hall[1],
    ...
    def getAvailableSeats(self, book_id):
        self.mycursor.execute(
            "select * from seats where Book_id = " + str(book_id))
        myresult = self.mycursor.fetchall()
        for seats in myresult:
            return seats[1]
    ...
    def get_booking_details(self, movie_id):
        available_bookings = []
        self.mycursor.execute(
            "select * from movie_booking where Movie_id = " + str(movie_id))
        myresult = self.mycursor.fetchall()
        for book_option in myresult:
            book_info = {}
            book_info['book_id'] = book_option[0]
    ...
    def getMovieFromBook(self, book_id):
        ...
        self.mycursor.execute(
            "select Movie_Id from movie_booking where Book_id = " + str(book_id))
        myresult = self.mycursor.fetchall()
        movieid = myresult[0][0]

        self.mycursor.execute(
            "select Movie_name from Movies where Movie_Id = " + str(movieid))
        myresult = self.mycursor.fetchall()
    ...
    def getMovieToreview(self, customer_id):
        customer_id = str(customer_id)
        self.mycursor.execute(
            "select Book_id from booked where customer_id = " + customer_id)
        myresult = self.mycursor.fetchall()
        movies_booked = {}
        for book_info in myresult:
            movies_booked.update(self.getMovieFromBook(book_info[0]))
        self.mycursor.execute(
            "select Movie_Id from reviews where customer_id = " + customer_id)
        myresult = self.mycursor.fetchall()
    ...
    def adminMovies(self, vals, addorupdate):
        try:
            if addorupdate == "Add":
                sql = "INSERT INTO Movies (Movie_name, Genres, Age) VALUES {}".format(
                    tuple(vals))
                ...
                sql = "UPDATE Movies SET Movie_name='{}', Genres='{}', Age='{}' WHERE Movie_Id={}".format(
    ...
    def adminhall(self, vals, addorupdate):
        try:
            if addorupdate == "Add":
                sql = "INSERT INTO hall (Hall_name, Available_seats, Location) VALUES {}".format(
                    tuple(vals))
                self.mycursor.execute(sql)
                self.mydb.commit()

            else:
                sql = "UPDATE hall SET Hall_name = '{}', Available_seats= '{}', Location='{}' WHERE Hall_id = {}".format(
                    vals[1], vals[2], vals[3], vals[0])
                self.mycursor.execute(sql)
                self.mydb.commit()
        except:
    ...
    def adminbooking(self, vals, addorupdate):
        ...
        try:
            if addorupdate == "Add":
                sql = "INSERT INTO booking (Movie_Id, Hall_id, date, Cost, Slots) VALUES {}".format(
                    tuple(vals))
                self.mycursor.execute(sql)
                self.mydb.commit()

            else:
                sql = "UPDATE booking SET Movie_Id = '{}', Hall_id= '{}', date='{}', Cost='{}', Slots='{}'   WHERE Movie_Id = {}".format(
    ...
    def adminseats(self, vals, addorupdate):
        try:
            if addorupdate == "Add":
                sql = "INSERT INTO seats (Book_id, available_seats) VALUES {}".format(
                    tuple(vals))
                self.mycursor.execute(sql)
                self.mydb.commit()

            else:
                sql = "UPDATE seats SET available_seats = '{}' WHERE Book_id = {}".format(
                    vals[1], vals[0])
                self.mycursor.execute(sql)
                self.mydb.commit()
        except:
            print("hello")
# <<<<<<< HEAD
# # >>>>>>> 45078ced5732bc158f5ed01fed93ea9def13a4be
# =======
